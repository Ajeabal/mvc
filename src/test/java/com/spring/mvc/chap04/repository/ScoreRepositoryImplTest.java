...
class ScoreRepositoryImplTest {
    ...
    // 단위 테스트 (Unit Test)
    // junit 5
    // 테스트에는 시나리오가 필요함 ex) A를 주면 B가 나온다.
    // 단언(Assertion) 기법 ~~해야 한다, ~~이다 (o)
    //                    ~~할 것이다, ~일 것 같다 (x)
    // GWT패턴 - Given, When, Then 패턴

    @Test
    @DisplayName("저장소에서 findAll 메서드를 호출하면 리스트가 반환되고, 해당 리스트에는 성적정보가 3개 들어 있어야 한다.")
    void findAllTest() {
        // GWT패턴
        // given: 테스트를 위해 주어진느 데이터 - parameter
    ...
    void saveTest() {
        // given
        Score score = new Score("새로운", 50, 60, 70, 4, 0, 0, Grade.B);
        // when
