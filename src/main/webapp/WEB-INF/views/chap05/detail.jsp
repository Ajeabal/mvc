            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <c:if test="${empty login}">
                        <a href="/members/sign-in">댓글은 로그인 이후 작성 가능합니다.</a>
                    </c:if>
...
<script>
    ...
    function renderReplies({replies, count, pageInfo}) {
        ...
        if (replies !== null && replies.length > 0) {
                ...
                tag += `
                    <div id='replyContent' class='card-body' data-replyId='\${rno}'>
                        ...
                        tag += (profile ? `<img class="reply-profile" src="/local\${profile}" alt="profile image">`
                                        : `<img class="reply-profile" src="/assets/img/anonymous.jpg" alt="anonymous">`);
                           tag += `<b>\${writer}</b>
                            </span>
            ...
            tag += "<div id='replyContent' class='card-body'>댓글을 달아 생각을 공유 해보세요</div>";
        }
        // 댓글 렌더링
    ...
    // 서버에 실시간으로 비동기 통신을 해 JSON을 받아오는 함수
    ...
    function makeReplyPostClickEvent() {
        ...
        $addBtn.onclick = e => {
            ...
            if (textVal.trim() === '' || textVal.trim() === null) {
                alert('댓글 내용은 필수값 입니다.');
                return;
            } else if (writerVal.trim() === '' || writerVal.trim() === null) {
                alert('댓글 작성자는 필수값 입니다.');
                return;
    ...
    // 댓글 삭제 + 수정모드 진입 이벤트 헨들러 등록 및 처리 함수
